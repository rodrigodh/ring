...
1. WebFetch multi-tenant.md: https://raw.githubusercontent.com/LerianStudio/marsai/main/dev-team/docs/standards/golang/multi-tenant.md
...
3. **If multi-tenant code exists → run compliance audit:**
   ...
   - Route ordemarsai: Auth MUST run before tenant middleware — per-route via `WhenEnabled` (not global `app.Use`)
...
4. **If multi-tenant code is MISSING entirely** → ISSUE-XXX (CRITICAL): "Service does not support multi-tenant mode. MUST run marsai:dev-multi-tenant."
