...
`<lint_command> 2>&1 | tee /tmp/marsai:lint-output.txt && echo "EXIT_CODE: $?"`
...
| Go | `general-purpose` or `marsai:backend-engineer-golang` |
| Security lints | `marsai:security-reviewer` for analysis first |
...
| Skill | When to use |
|-------|-------------|
| `marsai:dispatching-parallel-agents` | Pattern basis for this skill |
| `marsai:systematic-debugging` | If lint errors indicate deeper issues |
| `marsai:requesting-code-review` | After lint passes, before merge |

## Example Session

`/marsai:lint` → Run lint → 16 issues in 3 areas → Analyze streams (API: 5, Services: 8, Utils: 3) → Dispatch 3 parallel agents → All complete → Re-run lint → ✅ All pass.
