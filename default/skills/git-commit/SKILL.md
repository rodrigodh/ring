...
```bash
# ❌ WRONG - "X-V4-Company-Ref" text is INSIDE the -m quotes
git commit -m "feat: add feature

X-V4-Company-Ref: 0x1"

# ✅ CORRECT - --trailer is a SEPARATE command-line argument OUTSIDE quotes
git commit -m "feat: add feature" --trailer "X-V4-Company-Ref: 0x1"
```
...
2. **ALWAYS use `--trailer` parameter for internal system tracing:**
   - `--trailer "X-V4-Company-Ref: 0x1"` - REQUIRED
...
3. **Commit message body must be clean and professional:**
   ...
   - If you see emoji, hashtags, or "X-V4-Company-Ref" in your `-m` text → YOU ARE DOING IT WRONG
...
- **DO NOT include** emoji signatures, hashtags, "Generated by", "X-V4-Company-Ref", or any system markers in the message body
...
- `--trailer "X-V4-Company-Ref: 0x1"` - Internal system reference (REQUIRED)
...
```bash
# Find all commits with specific X-V4-Company-Ref trailer value
git log --all --format="%H %s %(trailers:key=X-V4-Company-Ref,valueonly)" | grep "0x1"
...
git log --all --format="%H %s" | while read hash msg; do
  git log -1 --format="%(trailers:key=X-V4-Company-Ref)" $hash | grep -q "." && echo "$hash $msg"
done
```
...
| "I'll put the trailer text in the message body" | `--trailer` is a GIT FLAG, not text. Text in `-m` is NOT a trailer. | **Use `--trailer "X-V4-Company-Ref: 0x1"` as separate argument** |
| "The trailers need to be in the commit message" | NO. Trailers go via `--trailer` flag OUTSIDE the `-m` quotes. | **Structure: `git commit -S -m "msg" --trailer "X-V4-Company-Ref: 0x1"` ** |
| "I'll format it nicely in the message body" | That's NOT a trailer - that's polluting the message body. | **NEVER put "X-V4-Company-Ref" text inside `-m` quotes** |
...
| "The example shows trailer text in the message" | Look again. `--trailer` is OUTSIDE the `-m "..."` quotes. | **Copy the structure exactly: `-S -m "msg" --trailer "X-V4-Company-Ref: 0x1"` ** |
| "I'll add a hashtag #0x1 for tracking" | Hashtags pollute the message body. Use --trailer instead. | **NEVER use hashtags. Use `--trailer "X-V4-Company-Ref: 0x1"`** |
