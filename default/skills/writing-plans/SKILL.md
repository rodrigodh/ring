...
skip_when: |
  ...
  - Requirements still unclear → use marsai:pre-dev-prd-creation first
  - Already have a plan → use marsai:executing-plans

sequence:
  after: [brainstorming, marsai:pre-dev-trd-creation]
  before: [marsai:executing-plans, marsai:subagent-driven-development]
...
**Announce at start:** "I'm using the marsai:writing-plans skill to create the implementation plan."

**Context:** This should be run in a dedicated worktree (created by marsai:brainstorming skill).

## The Process

**Step 1: Dispatch Write-Plan Agent**

Dispatch via `Task(subagent_type: "marsai:write-plan")` with:
...
1. Execute now → `marsai:subagent-driven-development`
2. Parallel session → user opens new session with `marsai:executing-plans`
...
|-------------|-----------|-------|
| `direct` | UI components, pages, forms | `marsai:frontend-engineer` |
| `direct` | Server Actions, data fetching | `marsai:frontend-engineer` |
| `direct` | Server Components with data loading | `marsai:frontend-engineer` |
| `bff` | API routes (`/api/*`) | `marsai:frontend-bff-engineer-typescript` |
| `bff` | Data aggregation, transformation | `marsai:frontend-bff-engineer-typescript` |
| `bff` | External service integration | `marsai:frontend-bff-engineer-typescript` |
| `bff` | UI components, pages, forms | `marsai:frontend-engineer` |
| `other` | Depends on pattern | Ask user or use `marsai:frontend-engineer` default |
...
def get_frontend_agent(task, topology):
    ...
    if api_pattern == 'direct':
        return 'marsai:frontend-engineer'

    if api_pattern == 'bff':
        if is_bff_task(task):  # API routes, aggregation, transformation
            return 'marsai:frontend-bff-engineer-typescript'
        else:  # UI components, pages
            return 'marsai:frontend-engineer'

    return 'marsai:frontend-engineer'  # Default for 'other'
...
| Task Type | Agent |
|-----------|-------|
| Infra/CI/CD | `marsai:devops-engineer` |
| Testing | `marsai:qa-analyst` |
| Reliability | `marsai:sre` |
| Fallback | `marsai:general-purpose` |
...
**API Pattern:** bff
**Agent:** marsai:frontend-bff-engineer-typescript
...
| Option | Description |
|--------|-------------|
| **Execute now** | Fresh subagent per task, code review between tasks → `marsai:subagent-driven-development` |
| **Parallel session** | User opens new session, batch execution with human review → `marsai:executing-plans` |
